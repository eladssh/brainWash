...
class UserDataManager:
    ...
    def create_user(username, onboarding_data):
        ...
        try:
            ...
            cursor2.execute("SELECT id FROM User WHERE username = ?", (username,))
...
class GoalManager:
    ...
    def create_initial_goals(user_id, onboarding_data):
        ...
        cursor.execute("""
            SELECT COUNT(*) FROM Goal WHERE user_id = ?
        """, (user_id,))
        
        if cursor.fetchone()[0] > 0:
            # Goals already exist, skip creation
            conn.close()
            return
    ...
    with tab1:
        with st.form("login"):
            ...
            if st.form_submit_button("Login", type="primary", use_container_width=True):
                if not username or not username.strip():
                ...
                elif UserDataManager.user_exists(username.strip()):
                    user = UserDataManager.get_user(username.strip())
                    ...
                    st.error("User not found! Please create an account in the 'New User' tab.")
        ...
        with st.form("signup"):
            ...
            if st.form_submit_button("Create Account", type="primary", use_container_width=True):
                if not new_username or not new_username.strip():
                    st.error("Please enter a username!")
                elif UserDataManager.user_exists(new_username):
...
def render_onboarding():
    ...
    # Safety check: if user already exists, skip to main app
    if UserDataManager.user_exists(st.session_state.username):
        user = UserDataManager.get_user(st.session_state.username)
        st.session_state.user_id = user[0]
        st.session_state.onboarded = True
        st.info("Your account already exists! Loading your profile...")
        time.sleep(0.5)
        st.rerun()
        return
